...
function [S, T, Va, V4]=intgrl1s(IOP,n_basis,n_pg_basis,alpha_basis,coef_basis,center_basis,type_basis,Za,fid)
    ...
    %Overlap, Kinetic & Potential integral
    ...
    for l=k:n_basis
        ...
        V4(j,i,k,l)=V4(i,j,k,l); % (ba|cd) = (ab|cd)
        V4(i,j,l,k)=V4(i,j,k,l); % (ab|dc) = (ab|cd)
        V4(j,i,l,k)=V4(i,j,k,l); % (ba|dc) = (ab|cd)
        % Where is (cd|ab) = (ab|cd)?
    end
    end
    end
    end
...
%         end
    ...
    function output(S,T,Va,V4,fid)
        ...
        fprintf(fid,'start of iterations for 1s shell');
        fprintf(fid,'S\n');
        temp=size(S);
        nsize=temp(1);
            for i=1:nsize
